...
public class RevelationFixMixinPlugin extends ApplyCheckMixinConfigPlugin {
                ...
                plugins.put("RevelationFixPlugin", new ILaunchPluginService() {
                    ...
                    public boolean processClass(Phase phase, ClassNode classNode, Type classType) {
                        ...
                        if (phase == Phase.BEFORE) {
                            if (name.equals("net/llenzzz/covenant_of_the_seven/CovenantOfTheSeven")) {
                            ...
                            if (name.startsWith("net/llenzzz/covenant_of_the_seven/mixin")) {
                                clearMixinClass(classNode);
                                shouldWrite.set(true);
                            } else if (name.startsWith("net/llenzzz/covenant_of_the_seven/")) {
                                if (removeEventBusSubscriberAnnotation(classNode.visibleAnnotations)) {
                                    shouldWrite.set(true);
                                }
                                if (removeEventBusSubscriberAnnotation(classNode.invisibleAnnotations)) {
                                    shouldWrite.set(true);
                                }
                                if (shouldWrite.get() && classNode.methods != null) {
                                    classNode.methods.forEach(methodNode -> removeSubscribeEventAnnotation(methodNode, shouldWrite));
                                }
                            }
                        }
    ...
    private static boolean removeEventBusSubscriberAnnotation(List<AnnotationNode> annotations) {
        return annotations != null && annotations.removeIf(annotationNode -> "Lnet/minecraftforge/fml/common/Mod$EventBusSubscriber;".equals(annotationNode.desc));
    }

    public static void removeSubscribeEventAnnotation(MethodNode methodNode, AtomicBoolean shouldWrite) {
        if (methodNode.invisibleAnnotations != null && methodNode.invisibleAnnotations.removeIf(node -> "Lnet/minecraftforge/eventbus/api/SubscribeEvent;".equals(node.desc))) {
            shouldWrite.set(true);
        }
        if (methodNode.visibleAnnotations != null && methodNode.visibleAnnotations.removeIf(node -> "Lnet/minecraftforge/eventbus/api/SubscribeEvent;".equals(node.desc))) {
            shouldWrite.set(true);
        }
    }
    ...
    public void postApply(String s, ClassNode classNode, String s1, IMixinInfo iMixinInfo) {
        if (!"com/Polarice3/Goety/common/entities/boss/Apostle".equals(classNode.name) || classNode.methods == null) {
            return;
        }
        classNode.methods.forEach(methodNode -> {
            if ("getTheData".equals(methodNode.name) && "(Lcom/Polarice3/Goety/common/entities/boss/Apostle;)F".equals(methodNode.desc)) {
                methodNode.instructions.clear();
                InsnList insnNodes = new InsnList();
                insnNodes.add(new VarInsnNode(Opcodes.ALOAD, 1));
                insnNodes.add(new MethodInsnNode(Opcodes.INVOKESTATIC, EVENT_UTIL_CLASS, "getData", "(Lcom/Polarice3/Goety/common/entities/boss/Apostle;)F", false));
                insnNodes.add(new InsnNode(Opcodes.FRETURN));
                methodNode.instructions.add(insnNodes);
            } else if ("setTheData".equals(methodNode.name) && "(Lcom/Polarice3/Goety/common/entities/boss/Apostle;F)V".equals(methodNode.desc)) {
                methodNode.instructions.clear();
                InsnList insnNodes = new InsnList();
                insnNodes.add(new VarInsnNode(Opcodes.ALOAD, 1));
                insnNodes.add(new VarInsnNode(Opcodes.FLOAD, 2));
                insnNodes.add(new MethodInsnNode(Opcodes.INVOKESTATIC, EVENT_UTIL_CLASS, "setData", "(Lcom/Polarice3/Goety/common/entities/boss/Apostle;F)V", false));
